-- Xóa hoàn toàn tính năng "Loại máy":
--  - Bảng device_types (danh sách loại máy quản lý được)
--  - Cột device_type trên repair_orders (ô "Loại máy")
--  - Cột compatible_models trên inventory_parts (ô "Tương thích")
-- GIỮ LẠI: cột device_model trên repair_orders (ô "Model máy") — app vẫn dùng.
-- App đã bỏ toàn bộ UI "Loại máy", không còn đọc/ghi trường device_type.
...
-- restore_store_data / clear_store_data đang tham chiếu bảng + cột vừa xóa
-- -> tạo lại (bỏ các tham chiếu cũ, vẫn giữ device_model trong repair_orders).
