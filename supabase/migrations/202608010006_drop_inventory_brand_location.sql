-- Loại bỏ cột brand (thương hiệu) và location (vị trí kho) khỏi inventory_parts
-- vì không còn dùng trong giao diện.

alter table public.inventory_parts
  drop column if exists brand,
  drop column if exists location;

-- restore_store_data đang tham chiếu 2 cột vừa xóa -> phải tạo lại (bỏ brand/location).
...
begin
  ...
  insert into public.inventory_parts (
    ...
    low_stock_threshold, barcode, imei, compatible_models,
  ...
  select x.id, x.store_id, x.name, x.sku, x.category_id, x.quantity, x.unit_cost,
    ...
    x.compatible_models, x.created_at, x.updated_at, x.deleted_at, x.deleted_by
  from jsonb_to_recordset(coalesce(j->'inventory_parts', '[]'::jsonb)) as x(
    ...
    barcode text, imei text, compatible_models text,
